...
# This is purely for testing purposes. This function is not expected to be
# called in production.


@books_api.route("/search/", methods=["GET"])
def google_books_testing():
    """
    Call this with parameters to make a call to Google Books API.
    Returns nothing if no correct arguments are given.

    This is a purely temporary routable function for use of testing the
    Google Books API calling system.
    ...
    # Otherwise, makes a call to the actual google books function and returns
    # what it gives back.
    return google_books_search(
        request.args.get("phrase", ""),
