...
const booksToAdd = [
  { id: "1", title: 'Book 1', author: 'Author 1', category: 'Fiction', summary: 'Summary 1', excerpt: 'Excerpt 1', image: 'Image 1', rating: 4 },
  { id: "2", title: 'Book 2', author: 'Author 2', category: 'Non-fiction', summary: 'Summary 2', excerpt: 'Excerpt 2', image: 'Image 2', rating: 3 },
];
...
describe('AddBooksOrMoveBooksToCategoryModal', () => {
  ...
  it('renders the modal with categories and books', () => {
    ...
    expect(screen.getByText('Select one or more categories to add the selected books to:')).toBeTruthy();
    ...
    booksToAdd.forEach(book => {
      expect(screen.getByText(`${book.title} by ${book.author}`)).toBeTruthy();
    });
  });
  ...
  it('shows error message when no categories are selected', async () => {
    ...
    expect(screen.getByText('Please select at least one category.')).toBeTruthy();
  });
  ...
  it('shows loading spinner when in loading state', async () => {
    ...
    mockAddBooks.mockImplementationOnce(() => new Promise(resolve => setTimeout(resolve, 1000)));
